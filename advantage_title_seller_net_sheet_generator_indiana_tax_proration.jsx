...
//
// NOTE: Owner's policy premium chart is carried over from the IHT tool for now.
// If Advantage has a different owner's policy chart, tell me and I'll swap the table.
...
// From AT schedule (effective 9/1/2025) based on your pasted table
const AT_SCHEDULE = {
...
(function runTestsOnce() {
  ...
  try {
    // Settlement split
    const t1 = calcAtSellerTitleFees({
      ...
      includeTitleSearchExam: false,
      includeClosingProcessing: false,
    });
    const settlement = t1.items.find((x) => x.label.includes("Settlement"));
    assert(settlement && settlement.amount === 225, "With-loan settlement is $450 total => seller $225");

    // Paid installments reduce debit
