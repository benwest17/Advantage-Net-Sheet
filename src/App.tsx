 ...
 * Single-file React component (Vite + TS) using Tailwind + framer-motion + lucide-react + jsPDF.
 *
 * Includes:
 * - Indiana tax proration (arrears)
 * - Advantage seller fee section (single price sheet, no county pricing changes)
 * - Optional Owner’s policy premium auto-calc (currently uses the same chart as IHT)
 *
 * IMPORTANT: Confirm whether Advantage uses the same Owner’s Policy chart as IHT.
 * If different, swap OWNER_POLICY_RANGE_CSV values.
 */
...
// -----------------------------
// Indiana Counties (for consistency / display)
// -----------------------------
...
// -----------------------------
// Owner’s Policy Premium (placeholder chart)
// -----------------------------

type OwnerPremiumChoice = "mid" | "low" | "high";
type OwnerPolicyRow = { lo: number; hi: number; min: number; max: number };

// NOTE: This is currently the same table used for IHT in your prior tool.
// If Advantage differs, replace this CSV with Advantage’s chart.
...
// -----------------------------
// Advantage Seller Fees
// -----------------------------

type TitleFeeItem = { label: string; amount: number };

type AdvantageFeeSchedule = {
  closingFeeWithLoanTotal: number; // from sheet
  closingFeeCashTotal: number; // from sheet
  purchaseClosingBuyer: number; // sheet: Purchase – Buyer 100
  purchaseClosingSeller: number; // sheet: Purchase – Seller 100
  docPrepFee: number; // attorney doc prep (if needed)
  cplSeller: number; // 25
  tieffSeller: number; // 5
  deedRecording: number; // 25
  mortgageRecording: number; // 55 (not included in seller list by default)
  otherDocsRecording: number; // 25
  simplifilePerDoc: number; // 4.25
  transferPlusSDF: number; // 30
};

const AT_SCHEDULE: AdvantageFeeSchedule = {
  closingFeeWithLoanTotal: 450,
  closingFeeCashTotal: 250,
  purchaseClosingBuyer: 100,
  purchaseClosingSeller: 100,
  docPrepFee: 90,
  cplSeller: 25,
  tieffSeller: 5,
  deedRecording: 25,
  mortgageRecording: 55,
  otherDocsRecording: 25,
...
type TitleFeeSettings = {
  ...
  includePurchaseClosingSeller: boolean;
  includeDocPrep: boolean;
  includeCPL: boolean;
...
function calcAdvantageSellerTitleFees(s: TitleFeeSettings): { items: TitleFeeItem[]; total: number } {
  const items: TitleFeeItem[] = [];
  const schedule = AT_SCHEDULE;
  ...
  if (s.includeSettlementFee) {
    const total = s.transactionType === "with_loan" ? schedule.closingFeeWithLoanTotal : schedule.closingFeeCashTotal;
    const sellerHalf = round2(total / 2);
    items.push({ label: "Settlement / closing fee (seller half)", amount: sellerHalf });
  }

  if (s.includePurchaseClosingSeller) {
    items.push({ label: "Closing (purchase) - seller", amount: schedule.purchaseClosingSeller });
  }

  if (s.includeDocPrep) {
    items.push({ label: "Document prep (attorney)", amount: schedule.docPrepFee });
  }
  ...
  if (s.includeDeedRecording) {
    items.push({ label: "Recording fee: deed", amount: schedule.deedRecording });
...
export default function App() {
  ...
  const [includePurchaseClosingSeller, setIncludePurchaseClosingSeller] = useState(true);
  const [includeDocPrep, setIncludeDocPrep] = useState(false);
  const [includeCPL, setIncludeCPL] = useState(true);
  const [includeTIEFF, setIncludeTIEFF] = useState(true);
  // Requested behavior (mirroring IHT request): do NOT auto-check these
  ...
  const sellerFeeCalc = useMemo(() => {
    return calcAdvantageSellerTitleFees({
      transactionType,
      county,
      useSimplifile,
      ownerPolicyPremium,
      includeSettlementFee,
      includePurchaseClosingSeller,
      includeDocPrep,
      includeCPL,
      includeTIEFF,
        ...
        <motion.div {...cardMotion} className="mb-6 flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-neutral-500">Settlement fee is split 50/50. Deed/Transfer are off by default.</p>
            </div>
          </div>
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            <p className="text-[11px] text-neutral-500 max-w-[360px] text-right">
              Estimate only. Actual prorations, premiums, recording charges, and settlement charges may differ based on county records and the final settlement statement.
            </p>
          </div>
        </motion.div>
          ...
          <motion.div {...cardMotion} className="lg:col-span-2">
            <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-black/5">
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <Field label="County" hint="Displayed on PDF (fees do not vary by county)">
                  <select
                    value={county}
              ...
              <SectionTitleFees
                ...
                includePurchaseClosingSeller={includePurchaseClosingSeller}
                setIncludePurchaseClosingSeller={setIncludePurchaseClosingSeller}
                includeDocPrep={includeDocPrep}
                setIncludeDocPrep={setIncludeDocPrep}
...
function SectionTitleFees(props: {
  ...
  includePurchaseClosingSeller: boolean;
  setIncludePurchaseClosingSeller: (v: boolean) => void;
  includeDocPrep: boolean;
  setIncludeDocPrep: (v: boolean) => void;
  includeCPL: boolean;
  ...
  return (
    <div className="mt-8 rounded-3xl bg-neutral-50 p-5 ring-1 ring-black/5">
      ...
      <p className="mt-1 text-sm text-neutral-600">
        Settlement/closing fee is split 50/50. Deed recording and Transfer/SDF are off by default (toggle if seller pays).
      </p>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            ...
            <Pill active={props.transactionType === "cash"} onClick={() => props.setTransactionType("cash")} label="Cash / Contract" />
          </div>

          <div className="mt-4 text-sm font-medium">County</div>
          <div className="mt-2 text-sm text-neutral-700">{props.county} County (fees do not vary by county)</div>

          <label className="mt-4 flex items-center gap-2 text-sm text-neutral-700">
            <input type="checkbox" checked={props.useSimplifile} onChange={(e) => props.setUseSimplifile(e.target.checked)} className="h-4 w-4 rounded border-neutral-300" />
        ...
        <div>
          <Field label="Owner’s policy premium (estimate)" hint="Auto from sale price unless turned off">
            <div className="space-y-3">
              <label className="flex items-center gap-2 text-sm text-neutral-700">
                <input type="checkbox" checked={props.autoOwnerPolicy} onChange={(e) => props.setAutoOwnerPolicy(e.target.checked)} className="h-4 w-4 rounded border-neutral-300" />
          ...
          <div className="mt-4 grid grid-cols-1 gap-2">
            <CheckRow label="Settlement / closing fee (seller half)" checked={props.includeSettlementFee} onChange={props.setIncludeSettlementFee} />
            <CheckRow label="Closing (purchase) - seller" checked={props.includePurchaseClosingSeller} onChange={props.setIncludePurchaseClosingSeller} />
            <CheckRow label="Document prep (attorney)" checked={props.includeDocPrep} onChange={props.setIncludeDocPrep} />
        ...
        <div className="sm:col-span-2 rounded-3xl bg-white p-4 ring-1 ring-black/5">
          ...
          <div className="mt-2 space-y-2 text-sm">
            ...
            <div className="border-t border-neutral-200 pt-2">
              <Detail k="Total Advantage seller fees" v={toMoney(props.sellerTitleFeesTotal)} strong />
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="mt-8 rounded-3xl bg-neutral-50 p-5 ring-1 ring-black/5">
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <TaxInstallment title="Spring installment" subtitle="Typically due May 10" paid={props.springPaid} onPaid={props.setSpringPaid} amount={props.springPaidInput} onAmount={props.setSpringPaidInput} />
          <TaxInstallment title="Fall installment" subtitle="Typically due Nov 10" paid={props.fallPaid} onPaid={props.setFallPaid} amount={props.fallPaidInput} onAmount={props.setFallPaidInput} />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="sticky top-6 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-black/5">
      ...
      <div className="mt-4 space-y-3">
        ...
        <Row k="Advantage title fees (seller)" v={`(${toMoney(round2(props.sellerTitleFeesTotal))})`} />
      ...
      <button onClick={props.downloadPdf} className="mt-6 w-full inline-flex items-center justify-center gap-2 rounded-2xl bg-neutral-900 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-neutral-800" type="button">
        <Download size={16} /> Download PDF
      </button>
    </div>
  );
...
(function runTestsOnce() {
  ...
  if (g.__AT_NET_SHEET_TESTS_RAN__) return;
  g.__AT_NET_SHEET_TESTS_RAN__ = true;

  try {
    // Settlement split 50/50
    const feesLoan = calcAdvantageSellerTitleFees({
      transactionType: "with_loan",
      county: "Marion",
      useSimplifile: false,
      ownerPolicyPremium: 0,
      includeSettlementFee: true,
      includePurchaseClosingSeller: false,
      includeDocPrep: false,
      includeCPL: false,
      includeTIEFF: false,
      includeDeedRecording: false,
      includeTransferFeeSDF: false,
    });
    const settlement = feesLoan.items.find((x) => x.label.includes("Settlement / closing fee"))?.amount ?? 0;
    assert(nearlyEqual(settlement, 225), "Seller half of $450 with-loan closing fee is $225");

    // Doc prep fee (optional)
    const feesDocPrep = calcAdvantageSellerTitleFees({
    ...
    const docPrep = feesDocPrep.items.find((x) => x.label === "Document prep (attorney)")?.amount ?? 0;
    assert(nearlyEqual(docPrep, 90), "Doc prep fee is $90 when enabled");


    // Tax proration: paid installment reduces debit
    const closing = new Date(Date.UTC(2026, 0, 10));
    ...
    // Owner policy table sanity
    const p = calcOwnersPolicyPremium(50_000, "mid");
    assert(p.min === 209 && p.max === 220, "Owner policy range at 50k");
  } catch (e) {
